...
/**
 * CS#19 — deterministic PRNG (mulberry32) so per-attempt randomization can be
 * replayed from a persisted seed (roadmap §26: same attempt ⇒ same order).
 */
...
/** Fisher–Yates driven by a seeded PRNG — same seed ⇒ identical order. */
...
/** Fresh seed for a new attempt's choice ordering (persisted on the attempt). */
