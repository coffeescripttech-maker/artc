const { prisma } = require('./node_modules/@aratc/database/dist/index.js');
...
(async () => {
  ...
  // Probe with org header (try program's own org, and first org)
  for (const orgId of [program?.organizationId, orgs[0]?.id]) {
    if (!orgId) continue;
